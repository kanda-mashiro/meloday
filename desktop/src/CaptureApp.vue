...
async function onCapture(): Promise<void> {
  ...
  if (!text) return
  label.value = ''
  const { error } = await appendInboxItem(text)
  if (!error) {
...
<template>
  <div class="cap" @keydown.esc="hideWindow">
    ...
    <!-- Capture -->
    <div v-else class="cap__capture">
      <span class="cap__hash">›</span>
      <input
        ref="inputEl"
        v-model="label"
        class="cap__input"
        type="text"
        placeholder="记点什么… 支持 #标签 和 13:30-14:00,回车存入 Inbox"
        autocomplete="off"
        @keydown.enter.prevent="onCapture"
      />
